...
"""
v019 update
idk wtf happened in v018 update
either v017 or v018 was the CT push for groups
just gotta do that on the lights then now..

made one whole light control function instead of 2... 
gotta combine the actual light changing function though... 
gotta make it multi setting and group vs light

i think i fixed the displaying and assigning of proper CT value of lights
bunch of funky math stuff... 

removed a 5 second pause when exiting the CT mode... that was really weird...

did a CT K value and made the direction work the right way (reverse slope was wierd)
    ...
    while exitvar == False:
        ...
        if(not GPIO.input(21)):
            if(display <= total):
                ...
                if(ctmode == 1):
                    if(mode == "g"):
                        g_ct_control(display)
                        name_array,total,lstate_a = get_group_names()
                    elif(mode == "l"):
                        l_control(num_lights[display-1]) #need to change to change Ct control
                        name_array,num_lights,lstate_a,total = get_light_names()
                refresh = 1 
                pos = display
            else:
...
def g_ct_control(group):
    ...
    while exitvar == False:
        ...
        if(bri_pre != rot_bri or refresh ==  1 ):
            ...
            display_2lines("Group " + str(group),"CT: " + str(int(tempcalc)) + "K",17)
            refresh = 0
        if(rot_bri != bri_pre and millsdiff > 250):
            if(rot_bri > 500):
                rotbri = 500
            hue_groups(lnum = group,lon = "true",lbri = "-1",lsat = "-1",lx = "-1",ly = "-1",ltt = "5", lct = rot_bri)
            bri_pre = rot_bri
            print rot_bri
            prev_mills = mills
        elif(millsdiff > 250): 
