...
"""
v015 update
changed the way the clock is handled. updates the time, only if the time actually changed. 
no sense burning away and updating the screen if there's no new minute right? 
mahooosive cpu load reduction. went from like 70% load to 3.4-8
no need for the one second timeout thing... the cpu rests for 10 ms after computing shit
10ms + whatever is good enough! very responsive since it dosn't have to wait for a redraw... 
might have to edit tht in the rest of the program...     

added it to all of the menus...

holy fuck, massive reduction in cpu load when idling...
why didn't i do this sooner?!?!?!?! 
kinda a pita to make sure it's coded... basically, if there's no change in the position, why update the screen?
there's no new information for the screen other than the clock... no point, just wasting cycles..

sooo much faster now!!!! its freaking awesome! 
