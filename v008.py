...
"""
v008 update
uses the functions built in 007 and now automatically retrieves group and light names from the hub
now when using the group or light modifier, you'll know exactly what you're controlling
and in case you have dead light spots (i.e. removed from system) then it'll skip over that as well. 

deleted a couple of functions that aren't being used anymore. 

also added to the brightness change function to get the state of the light before hand and update it 
changed representation from 0-250 to 0-100% 
same number of steps but makes more sense? now in 4% increments becuase 100/4 = 25 steps
...
def g_control(group):  
    ...
    brite = os.popen("curl -H \"Accept: application/json\" -X GET  http://192.168.1.144/api/0DEuqTaGHB5J2V72IzV1K-r3mwpf9ddjDkDDIRdzr/groups/" + str(group) + " | grep -o '\"bri\":[0-9]*' | grep -o ':.*' | tr -d ':'").read()
...
def l_control(light):  
    display_custom("loading brightness...")
    brite = os.popen("curl -H \"Accept: application/json\" -X GET  http://192.168.1.144/api/0DEuqTaGHB5J2V72IzV1K-r3mwpf9ddjDkDDIRdzr/lights/" + str(light) + " | grep -o '\"bri\":[0-9]*' | grep -o ':.*' | tr -d ':'").read()
    brite = int(brite)      #make integer
    if brite < 10 and brite >= 0:
        brite = 10
