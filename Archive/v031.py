...
def get_house_scene_by_group(scenenumber,ltt):
    #Get a fresh groups json file
    display_2lines("Grabbing","Group States",15)
    get_group_names() 
    cmdout = os.popen("cat groups").read()
    ...
    for x, v  in wat.items():
            ...
            #debugmsg v
    for x, v  in wat.items():
        display_2lines("Building Array for","Group " + str(x) + " of " + str(len(result_array)),15)
        try:
            bri_array.append(wat[x]['action']['bri'])
        except:
            #debugmsg("errored on item " + x)
            bri_array.append("-1")
        try:
            cmode_array.append(wat[x]['action']['colormode'])
        except:
            #debugmsg("errored on item " + x)
            cmode_array.append("-1")
        try:
            hue_array.append(wat[x]['action']['hue'])
        except:
            #debugmsg("errored on item " + x)
            hue_array.append("-1")
        try:
            ct_array.append(wat[x]['action']['ct'])
        except:
            #debugmsg("errored on item " + x)
            ct_array.append("-1")
        try:
            sat_array.append(wat[x]['action']['sat'])
        except:
            #debugmsg("errored on item " + x)
            sat_array.append("-1")
        try:
            xy_array.append(wat[x]['action']['xy'])
        except:
            #debugmsg("errored on item " + x)
            xy_array.append("-1")
    ...
    while index < len(result_array):    
        scenecmd = "curl --silent -H \"Accept: application/json\" -X PUT --data '{\"on\":" + str(lstate_a[index]).lower() + ",\"bri\":" + str(bri_array[index]) + ",\"sat\":" + str(sat_array[index]) + ",\"xy\":" + str(xy_array[index]) + ",\"transitiontime\":" + str(ltt) + ",\"hue\":" + str(hue_array[index]) + "}' " + api_url + "/groups/" + str(keyvalues[index]) + "/action"
        #display_2lines("Writing","Group " + str(index + 1) + " of " + str(len(result_array)),15)
        print(scenecmd)
        groupnum = index + 1
        sceneobj.write("echo \"Set Group " + str(keyvalues[index]) + " = " + str(result_array[index]) + "\"\n")
...
while True:
    ...
    if(old_display != display):   
        if(display == 1):
            ...
            display_2lines(str(display) + ". Record by Light","Scene 1",13)
        elif(display == 6):
