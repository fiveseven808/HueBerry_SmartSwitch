...
def get_house_scene_by_light(scenenumber):
    name_array,num_array,lstate_a,total = get_light_names
    #name_array = array of all the names of lights
    #num_array = array of all of the light numbers (that's the only number in quotes in this json)
    #lstate_a = state of each light
    #total = size of num_array
    #WRITE FILE HEADER
    #FILE HEADER
    #cat template >> new_script.py
    x = 0 
    bri_array = []
    ct_arraay = []
    x_array = []
    ...
    sat_array = []
    tt_array = []
    #state_array = lstate_a
    """
    use total to figure out how many while iterations
    while x < total:
        current_light = numarray[x]
        #this allows you to iterate through the bulbs
        try:
            bri_array.append(wat[current_light]['action']['bri'])
        except:
            #did not have the value
            bri_array.append(-1)
        try:
            ct_array.append(wat[current_light]['action']['ct'])
        except:
            #did not have the value
            ct_array.append(-1)
        x = x + 1
    ...
    while x < total:
        APPENDTOFILE
            ...
            curl --silent -H \"Accept: application/json\" -X PUT --data '{\"on\":" + str(lstate_a[x]) + ",\"bri\":" + str(bri_array[x]) + ",\"sat\":" + str(sat_array[x]) + ",\"xy\":[" + str(x_array[x]) + "," + str(y_array[x]) + "],\"transitiontime\":" + str(ltt) + ",\"hue\":" + str(hue_array[x]) + "}' " + api_url + "/lights/" + str(num_array[x]) + "/action
        x = x + 1
    """
...
while True:
    ...
    if(not GPIO.input(21)):
        if(display == 0):
            ...
            #print(debug)
            debugmsg("turning all lights after dinner")
        elif(display == 6):
            ...
            debugmsg("turning lights everything dim")
        elif(display == 8):
            pos = 0
            settings_menu()
        elif(display == 9):
